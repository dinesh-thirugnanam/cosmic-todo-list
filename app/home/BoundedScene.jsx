...
export default function BoundedScene({ nodes = [] }) {
  ...
  const pathPoints = useMemo(() => {
    ...
    for (let i = 0; i < numPoints; i++) {
      ...
      currentRadius +=
        minDistance / Math.max(1, Math.sin(angleStep) * currentRadius || 1);
  ...
  return (
    <>
      ...
      <group ref={groupRef} position={[centerX, centerY, 0]}>
        <mesh position={[0, 0, -0.1]} ref={planeRef} receiveShadow={true}>
        ...
        {nodes.map((node, index) => (
          <Node
            key={node.id}
            position={pathPoints[index] || new THREE.Vector3(0, 0, 0)}
            nodeData={node.props}
          />
        ))}
      </group>
      <KeyboardControls controlsRef={controlsRef} speed={3} zoomSpeed={0.05} />
      <OrbitControls
        ref={controlsRef}
        enableRotate={false}
        enablePan={true}
